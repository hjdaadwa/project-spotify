...
export default class TrackList {
    ...
    constructor(data, title, type) {
        ...
        this.tracksView = this.createTracks();
        this.container.append(...this.tracksView);
        
    }

    /**
     * Создание шаблона и заполнения статической информацией. Часть констурктора
     * @param {string} title 
     * 
     */
    ...
    /**
     * Создает треки в зависимости от типа. Собирает данные для плеера в audioElements
     * 
     */
    createTracks() {
        let tracks = [];
        if (this.type == 'playlist') {
            ...
            tracks = this.data.map((item, index) => {
                ...
                if (item.preview_url) {
                    API.get(`tracks/${item.id}?market=US`).then(data => {
